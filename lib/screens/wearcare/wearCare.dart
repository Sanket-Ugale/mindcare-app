...
//   const wearCare({super.key});

//   @override
//   State<wearCare> createState() => _wearCareState();
// }
...
class _wearCareState extends State<wearCare> {
  ...
  Future addData() async {
    ...
    // Add data for supported types
    // NOTE: These are only the ones supported on Android's new API Health Connect.
    // Both Android's Google Fit and iOS' HealthKit have more types that we support in the enum list [HealthDataType]
    // Add more - like AUDIOGRAM, HEADACHE_SEVERE etc. to try them.
    bool success = true;
    success &= await health.writeHealthData(
 ...
 Widget _contentDataReady() {
  ...
  for (HealthDataPoint dataPoint in _healthDataList) {
    if (dataPoint.type == HealthDataType.STEPS) {
      String dateKey = dataPoint.dateFrom.toLocal().toLocal().toString().split(' ')[0];
      stepsByDate[dateKey] = ((stepsByDate[dateKey] ?? 0) + int.parse(dataPoint.value.toString())) as int;
    }
  }
  ...
  return Column(
    ...
    children: [
      ...
      // Example for displaying blood glucose data
      // Container(
      //   padding: EdgeInsets.all(10),
      //   color: Colors.red,
      //   child: Text(
